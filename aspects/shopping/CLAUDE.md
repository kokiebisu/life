...
- **ショッピング DB**: `51f39ff99e804451a4f17d60f6869755`（[開く](https://www.notion.so/51f39ff99e804451a4f17d60f6869755)）
- data source: `collection://a9d23a4a-9e4b-4187-8521-b654bd8a3fd8`
...
1. **Web Search で商品を調べる** — 価格・商品ページURL・画像URL を確認する
2. **候補が複数ある場合（厳守）** — 表形式で特徴を並べ、**おすすめを明記してからユーザーに確認する**。確認後に登録する
3. **該当店舗の `stores/店舗名.md` に追記する** — ファイルがなければ新規作成
4. **Notion ショッピング DB（`51f39ff99e804451a4f17d60f6869755`）にレコードを作成する**
   - `notion-create-pages` で商品名・店舗・価格・URLをプロパティにセット
   - カバー画像に商品画像URLをセット
   - ページ本文にも商品画像を `![]()` で埋め込み、特徴をメモする
5. **`tasks.md` には入れない**（shopping/stores/ で管理するため）
6. ユーザーに「Notion と md に追加した」と1行で報告する
