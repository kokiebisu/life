...
### 2026-04-29 - 🔺 あいまい
- **Q: goroutineとOSスレッドの違いは？M:Nモデルとは？**
  - 詰まった内容: M:N の M/N を逆に説明（M個のOSスレッドにN個のgoroutine と回答）。スタックサイズを 1KB と回答（正: ~2KB）。切り替えコスト（ユーザー空間 vs カーネル切替）に言及なし
  - 正解ポイント: **M = goroutine 数、N = OSスレッド数**（M >> N、M個のgoroutineをN個のOSスレッドに多重化）。goroutine スタック ~2KB、OSスレッド ~1MB。goroutine はユーザー空間で切替（高速）、OSスレッドはカーネル切替（重い）

### 2026-04-24 - 🔺 あいまい
- **Q: goroutine と OS スレッドの違いは？M:N モデルとは？**
  - 詰まった内容: M:N モデルは答えられたが、細部に誤り (1) goroutine スタックサイズを 100KB と回答（正: ~2KB）、(2) OS スレッドも「Go ランタイムの中で作成」と説明（正: OS カーネルが管理、Go ランタイムは「借りる」）、(3) 切り替えコスト（ユーザー空間 vs カーネル切替）に言及なし
  - 正解ポイント:
    - goroutine = **ユーザー空間で管理される超軽量スレッド**（~2KB 初期、伸縮可能、ユーザー空間切替で高速）
    - OS スレッド = **カーネルが管理**（~1MB、カーネル切替で重い）
    - 作成可能数: goroutine は数十万〜数百万、OS スレッドは数千が限界
    - M:N = M個のgoroutineをN個のOSスレッドに多重化（GOMAXPROCS=N でコア数制御）
- **補足: channel の細部**
  - buffered channel もバッファ満杯になったらブロックする
  - 「受信側で close → 送信側が `ch <- v` で panic（send to closed channel）」が正確な機序
  - 二重 close も panic、close 済みチャンネルからの receive は zero value 返却（panic なし）
