...
1. `aspects/diet/gym-logs/` の直近3ファイルを確認して前回・前々回の種目・重量・部位を取得
   - **最新ログが昨日より古い場合（2日以上ギャップあり）は必ず実行（スキップ禁止）**: 以下の手順で Notion を確認する。**ローカルに空白日があっても「行っていない」と勝手に判断しない。**
     1. `bun run scripts/notion-list.ts --date YYYY-MM-DD` で各対象日を確認（ジム DB が含まれない場合もある）
     2. `notion-search` で `ジム M/D`（例: `ジム 3/20`）を検索
     3. 上記で見つからない場合でも**「エントリなし」と結論しない**。JST/UTC ズレで検索に引っかからないことがある。`notion-fetch` でジム DB（`collection://326ce17f-7b98-806a-be76-000b67b58628`）のスキーマを確認しつつ、ユーザーに「Notion のジム DB で該当日のエントリが見つかりません。URL を共有してもらえますか？」と確認する
