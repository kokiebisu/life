...
| 案 | 概要 | メリット | デメリット | 採用 |
|----|------|---------|-----------|------|
| A: freee 福利厚生のサイドバーに組み込む | 福利厚生プロダクトの一機能として提供 | 当初構想の「従業員が自由に受講するEラーニング」とは相性がよい | 構想が「人事担当者が新卒・部署・雇用形態などに応じて必須研修を割り当てる」方向へシフトしたため、福利厚生の自由受講ユースケースとずれた。組織図・従業員データとの連携も主軸になる | 却下 |
| B: 完全スタンドアローン（独自サブドメイン） | 独立 URL。人事労務からリダイレクト | freee 本体に影響を与えずに独立リリース・スケールが可能 | 独自ドメイン・独自リポジトリ・基盤連携・認証・運用・コードガイドラインを新規に整える必要がある。24卒・25卒を含む3名チームで並行して進めるには重い | 却下 |
| C: 人事労務のサイドバーに周辺プロダクトとして組み込む | 人事労務の UI 内に Eラーニングタブを追加 | 人事担当者が組織図や従業員データを見ながら研修管理まで完結できる。既存のコードガイドライン・基盤運用・当番制にも乗せられる | 人事労務のリリースサイクルに引きずられる可能性がある | **採用** |

**案 C の採用判断:**  
元々は福利厚生の一環として始まったが、最終的なニーズは「人事労務データを使って、対象者に必要な研修を割り当てる」方向へ変わった。そのため、ユーザー体験としても人事労務の周辺プロダクトに置く方が自然だった。加えて、チームは3名で、OEM提供元もリアーキ中のため追加依頼に消極的だった。人事労務の既存基盤・コードガイドライン・当番制運用に乗ることで、初期開発とリリース後の保守コストを抑える判断をした。

#### サーバー・リポジトリの 3択

| 案 | 概要 | メリット | デメリット | 採用 |
|----|------|---------|-----------|------|
| A: freee 福利厚生のリポジトリに組み込む | 既存の福利厚生サービスに機能追加 | 当初構想との距離が近い | 最終的な主要導線は人事担当者による研修管理であり、人事労務側の従業員・組織データ参照が必要になる。UI とデータの主戦場が人事労務側なのに、サーバーだけ福利厚生側になる分断が生じる | 却下 |
| B: 新規リポジトリ（独立マイクロサービス） | 独自の ECS/EKS・CI/CD・監視を持つ新サービス | 独立デプロイ・独立スケールが可能。障害が人事労務本体に波及しない | 基盤チームとの連携、認証、監視、運用、コードガイドライン整備が必要になる。3名チームかつリリース後は一旦一区切りにする前提では、初期開発と保守の両方で重すぎる | 却下 |
| C: 人事労務のリポジトリにモジュールとして組み込む（Rails Engine） | 人事労務の Rails モノリスに `Elearning::` 名前空間で組み込む | 既存インフラ・CI/CD・監視・コードガイドラインを即座に再利用。従業員マスタへのアクセスがシンプル | モノリスの肥大化。人事労務の障害が Eラーニングにも波及する可能性がある | **採用** |

**案 C の採用判断:**  
メリット・デメリットを踏まえた上で、以下 2点が決め手になった。

1. **独立サービス化の立ち上げコストが重い:** スタンドアローンにすると、認証・セッション・監視・CI/CD・コードガイドラインを新規に整える必要がある。24卒・25卒を含む3名チームでは、そこに工数を割くよりも、OEM連携と研修割り当ての価値検証に集中する方がよかった。

2. **リリース後の保守コストを抑えたかった:** OEM（LMSプロバイダー）がコア機能を担い、freee側は管理機能と組織同期が中心だった。Eラーニングはリリース後に一旦一区切りにする計画だったため、独立基盤を持つより、人事労務のアップグレード・当番制・監視に乗せる方が持続しやすい。

**受け入れたトレードオフ:**  
モノリスへの組み込みにより「人事労務本体の障害時に Eラーニングも影響を受ける」リスクを許容した。同一プロセス・同一 DB 接続プールである以上、Eラーニング障害が人事労務本体へ波及しないと完全には言えない。ただし、Rails Engine と `Elearning::` 名前空間によってルート・コントローラー・モデルの影響範囲を分け、通常のアプリケーションエラーが人事労務のコア機能へ直接影響しない構造にした。将来的に機能が複雑化・データが高感度化した場合の切り出しを見越し、人事労務コアモデルへの依存を最小限に抑えた。
...
| 選択肢 | 詳細な却下理由 |
|--------|--------------|
| **福利厚生プロダクト配下** | 当初の「従業員が自由に受講するEラーニング」という構想には合っていた。ただし、最終的なニーズは「人事担当者が組織図・部署・雇用形態を使って必須研修を割り当てる」方向に変わった。自由受講の福利厚生より、人事労務データに依存する研修管理の性質が強くなったため却下 |
| **独立マイクロサービス（新規リポジトリ）** | 独立ドメイン・独立リポジトリ・認証・CI/CD・監視・コードガイドラインを新規に整える必要がある。24卒・25卒を含む3名チームで、OEM提供元もリアーキ中のため追加依頼に消極的だった。Eラーニングはリリース後に一旦一区切りにする予定だったため、独立運用を増やすより人事労務の既存基盤と当番制運用に乗せる方がよいと判断 |
