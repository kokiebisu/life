...
- **食事エントリは原則 1 時間で登録する**（朝食・昼食・夕食すべて。メニューの簡易さに関わらず例外なし）
- 元のデイリープランに時間が記載されている場合はそれを踏襲する
- 食事エントリを登録・移動する際は、`fridge.md` で食材の在庫を確認する
...
**DB 優先度（高→低）:** events > todo > guitar = sound > routine > meals > groceries
...
エントリの時間を変更（延長・移動）した結果、**他の未完了エントリと重複が発生したら、重複エントリを翌日以降に移動する。**

### 手順

1. エントリの時間を変更した直後に `notion-list.ts --date YYYY-MM-DD --json` でその日の全エントリを取得
2. 変更したエントリの時間帯と重複する**未完了（ステータスが完了でない）**エントリを検出
3. 重複エントリについて:
   - 翌日の空き時間を確認し、同じ時間帯に移動を提案
   - ユーザーに確認の上、`notion-update-page` で日付を変更
   - guitar（Lesson）の場合は順番を維持する（Lesson 4 → Lesson 5 の順）
4. 移動後、移動先の日も重複がないか確認

### 判断基準

- **完了済みエントリ** → 何もしない（既に終わっている）
- **未完了 + 重複** → 翌日以降に移動
- **部分重複**（一部だけ被る） → 時間をずらせるなら shift、無理なら翌日移動

### 例

- Mid Week Service が 24:00 まで延長 → 20:00-23:00 の開発・Lesson・Devotion が重複 → 翌日に移動提案
...
- **既知ルート（下表）** はそのまま使う
- **未知ルート（下表にない場所）** は `travel-time.ts` で必ず計算する。デフォルト値（15分等）で済ませない
- **デイリープラン作成時も同様。** 場所付きイベントがある日は、前後の活動場所との移動時間を `travel-time.ts` で検証してから移動ブロックを配置する
...
`schedule.json` の `conflictRules` に基づく。詳細は `/from:notion` コマンド参照。
