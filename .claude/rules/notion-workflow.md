...
- カリキュラム DB は「カリキュラム」プロパティ（ギター / 音響）でレッスンを分類する
- **新規ページを作らない。** 日付未設定の既存 Lesson ページを探して日付をセットする
- `notion-sync-schedule.ts` が自動で処理する（`findNextLesson` で未スケジュール Lesson を検索、カリキュラム種別でフィルタ）
- ❌ 間違い: 「ギター受け取り」「弦の購入」等のレッスン以外の用事をカリキュラム DB に登録
- ✅ 正解: レッスン・練習以外は events DB / todo DB に入れる
