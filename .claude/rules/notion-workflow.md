...
## デボーション MD → Notion 登録（厳守）

デボーション MD ファイルを新規作成または更新した際は、必ず対応する Notion ページを確認・登録すること。

1. `bun run scripts/notion-list.ts --db devotion --date YYYY-MM-DD` で既存エントリを確認
2. エントリがなければ `notion-add.ts --db devotion --title "デボーション" --date YYYY-MM-DD --start HH:MM --end HH:MM` で作成
3. `notion-update-page` で `Book` / `Chapter`（数字）/ `icon: 🙏` を設定
4. `notion-update-page` の `replace_content` でページ本文を書き込む
