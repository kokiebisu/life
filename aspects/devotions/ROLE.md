...
- **開始時刻を記録する**（`TZ=Asia/Tokyo date +"%H:%M"` で取得）
...
- **章全文・複数節を引用する場合は、節ごとに1行ずつ `> **N** 本文` の形式で表示する**（節番号を太字、N は節番号）。長い blockquote にまとめて節が識別できない状態にしない
- **数節のみの引用（Key Verses 等）も、複数節にまたがる場合は節ごとに改行して節番号を明記する**
...
**Scripture:** 書籍名XX章 | **Key Verses:** X, Y, Z
...
3. **Book（書籍名）と Chapter（数値）を設定する** — 選択肢にない書籍は `notion-update-data-source` で追加してから設定する
4. **icon を 🙏 に設定する**
5. **タイトルを「デボーション」に正規化する** — 仮置き時の表記揺れ（「朝デボ」「devotion」等）を統一する
6. **日付の start/end を実際のセッション時刻で更新する** — 仮置き時刻はあくまで予定。開始時刻（セッション開始時に記録）と終了時刻（現在時刻）で上書きする

これにより、カレンダーに仮置きしたエントリを `/devotion` 完了後に自動で enrich できる。新規作成・仮置き enrich どちらのフローでも同じプロパティが設定される。

### 進め方

- 毎回次の章に進む（箴言16章 → 17章、または別の書籍に移る場合もある）
- **開始時に必ず `Glob` で `aspects/devotions/2*.md` を検索し、最新ファイルを読んで前回の書籍・章を確認する**（推測しない）
- 箴言に限らず、どの書籍でも扱う
