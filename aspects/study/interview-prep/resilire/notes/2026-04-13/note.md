...
1. Goのエラーハンドリングで `%w` と `%v` の違いは？`errors.Is` と `errors.As` の使い分けは？
2. Auditカラム（created_by等）でIDを使い文字列を使わない理由は？GDPR対応との関係は？
3. 災害アラートシステムでDLQを使う理由は？即時リトライとのトレードオフは？

## 🔁 復習で詰まったところ

### 2026-04-27 - 🔺 あいまい
- **Q: Auditカラム（created_by等）でIDを使い文字列を使わない理由は？GDPR対応との関係は？**
  - 詰まった内容: ON DELETE CASCADE を「ユーザー削除依頼への対応手段」と捉えていた。soft delete と SET NULL の位置関係も曖昧
  - 正解ポイント: Resilire の結論は **soft delete + nullable FK**。users は物理削除せず `deleted_at` + 個人情報 NULL 化（UPDATE）。CASCADE は監査ログごと消えるので危険、SET NULL は物理削除時の保険として設定する位置付け。soft delete が主役

### 2026-04-13
- **Q: errors.Is と errors.As の使い分けは？それぞれの第2引数は何を渡す？**
  - 詰まった内容: 区別が最初曖昧だった
  - 正解ポイント: struct かどうかで切り分ける。sentinel error → `errors.Is`、カスタム型（struct）→ `errors.As`。`errors.As` の第2引数は `&myErr`（ポインタのポインタ）
- **Q: typed nil の問題・理由・対策を一連で説明できる？**
  - 詰まった内容: 最初から流暢に説明できなかった
  - 正解ポイント: interface は (型情報, 値) のペアを持つ。型情報があると nil 判定が false になる（typed nil 問題）。対策: nil を返すときは必ず `return nil` と明示する（typed nil の変数を返さない）
- **Q: テナント・サプライヤー・施設の関係を、Resilire 文脈で説明できる？**
  - 詰まった内容: 概念整理ができていなかった
  - 正解ポイント: テナント = Resilire の顧客企業、サプライヤー = テナントの取引先、施設 = サプライヤーが持つ工場・倉庫（被災判定に使う）。通知先はテナントのユーザー（担当者）
- **Q: 設計の選択肢を聞かれたときの面接での話し方は？**
  - 詰まった内容: DLQ を提案した後に「他の方法は？」と聞かれて止まった
  - 正解ポイント: 自分から複数の選択肢を出して比較する習慣をつける。「他には X・Y・Z があり、DLQ を選んだ理由は〜」と自分から言い切る
