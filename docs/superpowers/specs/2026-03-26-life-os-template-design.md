...
- **Scripts** read `life.config.json` at runtime and skip operations for disabled aspects. Example pattern:
  ```ts
  import config from '../life.config.json'
...
- **`bun run setup`** is the only writer of this file.
