---
name: event
description: イベント・予定を Notion カレンダーに登録するとき。飲み会・会議・外出など日時が決まっている予定の登録に使う。移動時間・重複チェックも自動処理する。
---
...
1. **現在のカレンダーを確認する**
   ```bash
   bun run scripts/notion/notion-list.ts --days 7 --json
   ```
...
3. **日時を決定する** — 以下を考慮:
   ...
   - **理想のルーティン** — `aspects/daily/` の過去ログを参照して空き時間を判断する
...
5. **Notionに登録する**
   ```bash
   bun run scripts/notion/notion-add.ts --title "イベント名" --date YYYY-MM-DD --start HH:MM --end HH:MM
   bun run scripts/notion/notion-add.ts --title "イベント名" --date YYYY-MM-DD --allday  # 終日イベント
   ```
...
- DB が違っても（routine / meals / events / todo）時間が被ったらNG
- 重複がある場合は前後どちらかにずらす
...
**ルーティン活動（開発・ジム等）は devotion DB に入れる:**
- 場所の指定（図書館、カフェ等）やサブタイトルがついても、活動の本質が routine なら devotion DB
