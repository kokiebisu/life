...
- **更新タイミング**: 買い出し後、週次献立作成時、ユーザーが在庫について言及したとき、**食事メニューの移動・削除・差し替え時**
- **消費予定アノテーション（必須）**: 献立を作成・変更したら、`fridge.md` の各アイテムに「いつ・何に使うか」を注記する。**未使用** のアイテムが一目でわかるようにし、ペリシャブルな未使用品は献立に組み込む。不足する食材も同時に可視化する
- **買い出し量 = 献立の必要量（厳守）**: 買い出しリストは献立から逆算し、**使い切れる量だけ買う**。余りが出る買い方をしない。献立に入っていない食材は買わない。逆に、後から無理に献立にねじ込む必要が出たら、それは買い出し計画の失敗
...
**Notion-flavored Markdown 版（`replace_content` にそのまま使える）:**

```markdown
::: callout {icon="📋" color="green_bg"}
**[クラシル](https://レシピURL)** | 調理時間 <span color="orange">**20分**</span>
:::
---
### 🥗 材料（1人前）
- **鶏むね肉** 150g
...
2. フライパンで焼く
### 💡 コツ・ポイント
> むね肉は下味をつけると柔らかくなる<br>火加減は中火でじっくり
```

スキルテーマがある場合は末尾に追加:
```markdown
::: callout {icon="🎯" color="blue_bg"}
**🎯 今週のスキルテーマ**<br>焼く - フライパンの火加減
:::
```
...
3. 上記 Markdown テンプレートに従って内容を組み立てる
...
食事メニューを**移動・削除・差し替え**したら、**以下を必ず連動更新する:**
1. **`fridge.md`** — 消費予定の日時・残数を修正（朝食削除→食材が余る、時間変更→注記を修正）
2. **買い出しリスト（Notion groceries ページ）** — スクリプトで再生成
...
**Step 3: 余った食材の自動処理**（核心）

1. `fridge.md` を参照し、元の食事で使う予定だった食材を特定
2. 食材ごとに以下の優先順で自動判断:

| 優先度 | 条件 | 処理 |
|--------|------|------|
| 1 | 他の日にも消費予定がある | 消費予定から該当行だけ削除、残り数量を更新 |
| 2 | ペリシャブル（生鮮品）で翌日以内に使い切る必要あり | 翌日のメニューに自動組み込み（朝食の味噌汁の具に追加等） |
| 3 | 冷凍可能（肉・魚・パン等） | fridge.md に「冷凍」と記載、消費予定を後ろにずらす |
| 4 | 日持ちする（乾物・缶詰・玄米パック等） | 消費予定を削除して「持越」に変更 |

3. `fridge.md` の消費予定テーブルを更新
4. 翌日のメニューに食材を組み込んだ場合は、daily ファイル + Notion meals も更新
...
食材: ニラ1束 → 2/20朝味噌汁でも使う、問題なし
      卵2個 → 他の日にも予定あり、消費予定更新
```

**外食差し替え: 「今日の夜、〇〇と飲みに行く」**
```
処理: 自炊メニュー → 外食（〇〇と野毛）（kcal: —）
食材: 鶏むね肉150g → ペリシャブル、冷凍処理
      ブロッコリー → 翌日の味噌汁に組み込み
```
