...
**加えて:**  
- 福利厚生クラスタは SOC1 準拠ではなかった（人事労務クラスタは準拠済み）
- SOC1 準拠は必須要件ではないが、今後のセキュリティ審査や顧客要件の観点でプラスに働く可能性があった（nice to have）
...
効果:
  ...
  - SOC1 準拠レベルに自動引き上げ（nice to have として達成）
...
### 1. Blue/Green 移行における Cron 二重起動リスクをどう排除したか

**採用した方式:**  
Blue/Green デプロイを採用した。ただし標準的な Blue/Green（数時間〜1日の同時稼働期間を設ける）ではなく、以下の 2つの工夫を加えることで Cron の二重起動リスクを設計段階で排除した。

**問題の構造:**  
福利厚生サービスには「スケジュールされた Cron ジョブ」がある。例：毎日 02:00 に「月次請求レポート生成」「未払いの通知メール送信」が走る。Blue（旧クラスタ）と Green（新クラスタ）が同時に稼働している時間帯にこの時刻が来ると、両方で同じ Cron が実行される。

- **メール重複送信:** 「今月のご請求額のお知らせ」が 2通届く
- **DB の重複処理:** 月次集計バッチが 2回実行され、数値がおかしくなる

**工夫 1: Cron 実行時間帯の集約（移行前に事前デプロイ）**  
全 Cron の実行スケジュールを 04:00 以降に変更した。これにより「深夜 03:00〜04:00」が Cron ゼロ時間帯になる。

**工夫 2: 切り替え後に Blue を即時停止**  
ALB のトラフィックを Green に切り替えた直後、Blue（旧クラスタ）の replicas を 0 に落として即時停止した。Blue/Green の同時稼働時間を最小化することで、Cron ゼロ時間帯の短い窓の中に収めた。

```
移行ウィンドウのタイムライン:
  03:00〜04:00: Cron ゼロ時間帯（全 Cron を 04:00 以降に移動済み）
      ↓
  Green 起動・疎通確認
      ↓
  ALB 切り替え（Blue → Green）
      ↓
  Blue を即時停止（replicas: 3 → 0）
      ↓
  04:00: 最初の Cron が Green で実行（Blue はすでに停止済み）
```

**この設計の利点:**  
- Cron の二重起動が「設計上不可能な状態」になる（分散ロックのような「実装が正しければ二重起動しない」ではなく、タイムライン上で物理的に同時稼働しない）
- 既存コードへの変更は「Cron スケジュール時刻の修正」のみ（最小スコープ）
- ダウンタイムゼロを維持しつつ、Cron 安全性を確保
...
| SOC1 準拠（nice to have） | 準拠レベルへ引き上げ | **達成** |
| 移行後の異常アラーム | 0件 | **0件** |
