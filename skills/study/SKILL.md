...
- 本なし: `aspects/study/{category}/notes/YYYY-MM-DD-1.md`
- **本あり: `aspects/study/{category}/{本のタイトル}/YYYY-MM-DD-1.md`**（フラット・`notes/` なし）
- 同日に複数セッションある場合は連番: `YYYY-MM-DD-2.md`、`YYYY-MM-DD-3.md` …

本のタイトルはフロントマターの `book:` と完全一致させる（例: `システム設計の面接試験`）。Chapter はフロントマターに記録するのみで、ファイル名・フォルダ名には含めない。
...
7. **完了報告**:
   ...
   Notion ✅ / ローカル MD ✅
   📄 aspects/study/{category}/{本のタイトル もしくは notes}/{filename}.md
   ```
