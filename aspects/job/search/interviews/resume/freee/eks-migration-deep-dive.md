...
結果として、ダウンタイムなしで production 移行できました。今やり直すなら、CronJob suspend、DB lock、idempotency key、rollback runbook をより明示的に整えて、二重起動と戻し手順をさらに堅くします。」

### 深掘り版

- 判断軸: 独自クラスタ運用を続けるコスト、監査/権限管理の統一、チーム規模に合う運用責任
- 検討した選択肢: 独自 EKS 継続、人事労務クラスタへの namespace 統合、より大きな基盤再設計
- 採用理由: 小さなチームで運用責任を持ち続けるより、既存の人事労務基盤に寄せる方がプロダクト開発に集中できる
- トレードオフ: 統合先クラスタの影響範囲は広がるため、namespace、RBAC、移行手順で境界を作る必要がある
- 一番のリスク: Cron 二重起動。メール二重送信や外部連携の重複実行につながる
- 今なら改善: CronJob suspend、DB lock、idempotency key、rollback runbook、NetworkPolicy / ResourceQuota の明文化
